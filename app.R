...
server <- function(input, output) {
  ...
  output$wdPlot <- renderPlot({
    ...
    if(input$keywords %in% dt$word){
      ...
      cols<-randomColor(length(unique(dt$word)), luminosity = "dark")
      
      match_value<-match(input$keywords, dt$word)
      
      cols[match_value]<-"purple"
        
      print(ggplot(dt, aes(label = word, size=freq,
                                     color = cols, random.order = FALSE, ordered.colors=TRUE
      )) +
        geom_text_wordcloud_area(shape = input$shapes) +scale_size_area(max_size = input$max_size) +
        theme_minimal()
      )
    }else{
      print(ggplot(dt, size=1.6, aes(label = word, size=freq,
